...
Thanks for taking the time to contribute to `iamenr0s.ansible_role_k8s`!
...
# Full test against the default distro (almalinux10)
molecule test
...
Molecule uses Podman locally and Docker in CI. The CI matrix covers Debian, Ubuntu, AlmaLinux, Rocky Linux, and Fedora — changes must not break any supported distro (see `meta/main.yml` for the supported platform list).
